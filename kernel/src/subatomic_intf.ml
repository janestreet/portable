...
module type Subatomic = sig
  ...
  (*_ Templated atomic shared operations, e.g. [Subatomic.get [@synchro atomic]] *)
    include%template
      S [@synchro.explicit sync sync] [@mode.explicit contended] with type 'a t := 'a t
    (** @inline *)

  module%template Shared : sig
    (*_ Untemplated atomic shared operations, e.g. [Subatomic.Shared.get] *)
    include
      S [@synchro.explicit unsync sync] [@mode.explicit contended] with type 'a t := 'a t
    (** @inline *)
  end
